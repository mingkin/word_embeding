...
def main():
    ...
    for i in range(len(lookup)): #transform M to dictionary structure
        D[i] = Counter()
        if i == len(lookup) - 1:
            if M[lookup[i] + j] > 0:
                D[i].update({j: M[lookup[i] + j]})
            break
        for j in range(lookup[i+1] - lookup[i]):
            if M[lookup[i] + j] > 0:
                D[i].update({j: M[lookup[i] + j]})
    ...
    for i in range(len(top_buffer)): #remove tmpfiles
